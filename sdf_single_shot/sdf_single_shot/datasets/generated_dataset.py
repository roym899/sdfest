...
class SDFVAEViewDataset(torch.utils.data.IterableDataset):
    ...
    def _generate_sample(self) -> Tuple:
        ...
        if self._pointcloud:
            # TODO call depth_to_pointcloud instead
            indices = torch.nonzero(depth, as_tuple=True)
            depth_values = depth[indices]
            points = torch.cat(
                (
            ...
            if self._normalize_pose:
                points, centroid = pointset_utils.normalize_points(
                    points
                )
                position -= centroid  # adjust target

            sample["pointset"] = points
