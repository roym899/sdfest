...
if __name__ == "__main__":
    ...
    interval = (
        len(nocs_list) // (opt.num_agent - 1) if opt.num_agent > 1 else len(nocs_list)
    )
    task_range = nocs_list[
        interval * opt.id_agent : min(interval * (opt.id_agent + 1), len(nocs_list))
    ]
    ...
    for file_name in tqdm.tqdm(task_range):
        ...
        image_name = (
            pose_file["image_path"].replace(
                "data/real/test", opt.dataroot + "/real_test/"
            )
            + "_color.png"
        )
        image = cv2.imread(image_name)[:, :, ::-1]  # bgr to rgb
        ...
        for id, class_pred in enumerate(pose_file["pred_class_ids"]):
            ...
            # mask image, based on bounding box
            image_mask = image.copy()  # (rows, cols, 3,)
            image_mask[masks[:, :, id] == 0, :] = 255  # white background outside mask
            image_mask = image_mask[
                bbox[0] : bbox[2], bbox[1] : bbox[3], :
            ]  # (bb_rows, bb_cols, 3,)

            # convert image to float -1 to 1,
            A = (
                torch.from_numpy(image_mask.astype(np.float32))
                .cuda()
                .unsqueeze(0)
                .permute(0, 3, 1, 2)
                / 255
            ) * 2 - 1  # (1, 3, bb_rows, bb_cols)

            _, c, h, w = A.shape
            s = max(h, w) + 30  # add 15 pixel padding
            A = F.pad(
                A,
                [
                    (s - w) // 2,
            ...
            A = F.interpolate(
                A, size=opt.target_size, mode="bilinear"
            )  # (1, 3, target_size, target_size)

            # target_size is 64 -> 64x64 image with some padding around all edges

            state_history, loss_history, image_history = models[class_pred - 1].fitting(
                A
            )
            if opt.vis:
                # Use NOCS's prediction as reference for visualizing pose error as the resuls are not matched to GT's order.
                models[class_pred - 1].visulize_fitting(
                    A,
                    torch.tensor(pose_file["pred_RTs"][id]).float().unsqueeze(0),
                    state_history,
                    loss_history,
                    image_history,
                )
            ...
            states = voting(
                loss_history,
                state_history,
                category=categories[class_pred - 1],
                topk=5,
                thresh=10,
            )
            pose_file["pred_RTs_ours"][id][:3, :3] = scipy_rot.from_euler(
                "yxz", states[:3], degrees=True
            ).as_dcm()[:3, :3]
            ...
            mat = np.array(
                [
                    [
                        states[5] * np.cos(angle),
                        -states[5] * np.sin(angle),
                        states[5] * states[3],
                    ],
                    [
                        states[5] * np.sin(angle),
