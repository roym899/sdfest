...
class Mapping():
    ...
    def Cloud2Image( self, cloud_in, dbg_vis=False ):
        ...
        if len(cloud_color1) == len(cloud_np):
            ...
            for i, pix in enumerate(cloud_mapped.points):
                if pix[0]<self.width and 0<pix[0] and pix[1]<self.height and 0<pix[1]:
                    img[int(pix[1]),int(pix[0])] = int(255.0*((d_max-cloud_np[i,2])/base))
            img = cv2.merge((img_zero,img,img_zero))
            
            if dbg_vis:
                for i, pix in enumerate(cloud_mapped.points):
                    value = int(np.clip( 200.0*((d_max-cloud_np[i,2])/base), 0, 255 ))
                    cv2.circle(img, (int(pix[0]),int(pix[1])), 1, (0,value, 0), -1, cv2.LINE_AA )
...
def applyHPR( pcd, viewpoint=np.array([0.,0.,0.]), hpr_param=50 ):
