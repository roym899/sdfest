...
def compute_degree_cm_mAP(
    ...
    synset_names,
    log_dir,
    degree_thresholds=[360],
    shift_thresholds=[100],
    ...
    output_path = os.path.join(
        log_dir,
        prefix + "mAP_{}-{}cm.png".format(shift_thres_list[0], shift_thres_list[-2]),
    )
    ax_rot.legend()
...
def draw(image, RTs, models, class_ids, misses, intrinsics, save_path="bbox.png"):
    ...
    for RT, model, cls, miss in zip(RTs, models, class_ids, misses):
        ...
        projected_axes = calculate_2d_projections(transformed_pts, intrinsics)
