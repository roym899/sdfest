...
class ShapeNetTensorRenderer:
    ...
    def load_object(self, obj_path, texture_path, scale=1):
        if texture_path != '':
            is_texture = True
        ...
        size = np.max(np.linalg.norm(mesh.vertices, axis=1))
    ...
    def load_objects(self, obj_paths, texture_paths, scale=1, colors=[[0.9, 0, 0], [0.6, 0, 0], [0.3, 0, 0], [0.4, 0, 0]]):
        self.colors = colors
        sizes = []
        for i in range(len(obj_paths)):
            size = self.load_object(obj_paths[i], texture_paths[i], scale)
            self.instances.append(len(self.instances))

        # print(self.instances)
        # print(len(self.VAOs))

    def load_objects_pn(self, obj_paths, texture_paths, scale=1, colors=[[0.9, 0, 0], [0.6, 0, 0], [0.3, 0, 0], [0.4, 0, 0]]):
        self.colors = colors
        sizes = []
        for i in range(len(obj_paths)):
            size = self.load_object(obj_paths[i] + '/models/model_normalized.ply', texture_paths[i], scale)
            self.instances.append(len(self.instances))   
