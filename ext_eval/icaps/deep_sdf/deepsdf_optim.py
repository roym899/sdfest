...
class deepsdf_optimizer():
    def __init__(self, decoder, optimize_shape=False, lr=0.002):
        ...
        if optimize_shape:
            self.d_latent = torch.zeros((1, 256), dtype=torch.float32, requires_grad=True, device=0)
            self.optimizer = optim.Adam([self.dpose, self.d_latent], lr=0.002)
        else:
            self.d_latent = torch.zeros((1, 256), dtype=torch.float32, requires_grad=False, device=0)
    ...
    def decode_sdf(self, latent_vector, points, clamp_dist=0.1, MAX_POINTS=100000, no_grad=False):
    ...
    def compute_dist(self, d_pose, T_oc_0, ps_c, latent_vector):
        ...
        ps_o = torch.mm(Oplus(T_oc_0, d_pose, gpu=True), ps_c.permute(1, 0)).permute(1, 0)[:, :3]
        ...
        rotm_tensor = torch.from_numpy(rotm_obj2shapenet).float().cuda()
        points_obj_shapenet = torch.mm(rotm_tensor, points_obj_norm.permute(1, 0)).permute(1, 0)  #np.dot(rotm_obj2shapenet, points_obj_norm.T).T
    ...
    def eval_latent_vector(self, T_co_0, ps_c, latent_vector):
        ...
        dist, size_est = self.compute_dist(self.dpose.detach(), T_oc_0, ps_c, latent_vector)
    ...
    def refine_pose(self, T_co_0, ps_c, latent_vector, steps=100, shape_only=False):
        ...
        for i in range(steps):
            if shape_only:
                self.adjust_learning_rate(init_lr, self.optimizer, i, decreased_by, adjust_lr_every)
            self.optimizer.zero_grad()

            dist, size_est = self.compute_dist(self.dpose, T_oc_0, ps_c, latent_vector + self.d_latent)
            dist_target = torch.zeros_like(dist)
            dist_target = dist_target.cuda()
            
            loss = self.loss(dist, dist_target) + 1e-4 * torch.mean((latent_vector + self.d_latent).pow(2))
            loss.backward()
        ...
        # print('step: {}, loss = {}'.format(i + 1, loss.data.cpu().item()))
        T_oc_opt = Oplus(T_oc_0, self.dpose, gpu=True)
        ...
        return T_co_opt, dist, latent_vector_opt, size_est.detach().cpu().numpy(), loss.data.cpu().item()

    
    def adjust_learning_rate(self, initial_lr, optimizer, num_iterations, decreased_by, adjust_lr_every):
