...
from ..config.config import (cfg, cfg_from_file, get_output_dir,
                           write_selected_class_file)
# from ..datasets.render_shapenet_dataset import *
from ..models.pointnet2_msg import *
from ..models.aae_models import *
from ..utils.compute_farthest_distance_numba import get_bbox_dist, get_max_dist
...
class PoseRBPF:
    ...
    def set_target_obj(self, target_object):
        ...
        # print
        print(
            "target object class is set to {}".format(self.target_obj_cfg.PF.TRACK_OBJ)
        )
    ...
    def visualize_roi(
        ...
        if show_gt:
            rect = patches.Rectangle(
                (
                    gt_bbox_center[0] - 0.5 * gt_bbox_size,
        ...
        for bbox_center, bbox_size in zip(est_bbox_center, est_bbox_size):
            rect = patches.Rectangle(
                (bbox_center[0] - 0.5 * bbox_size, bbox_center[1] - 0.5 * bbox_size),
                bbox_size,
                bbox_size,
                linewidth=0.5,
                edgecolor=color,
        ...
        if error:
            ...
            plt.annotate(
                "step {} - t err:        {:.3f} cm".format(
                    step, self.log_err_t[-1] * 100
                ),
                xy=(0.15, z_show[0]),
                xycoords="axes fraction",
            )
            plt.annotate(
                "step {} - R err:        {:.3f} deg".format(step, self.log_err_r[-1]),
                xy=(0.15, z_show[1]),
                xycoords="axes fraction",
            )
            plt.annotate(
                "step {} - max similarity:   {:.2f}".format(step, self.log_max_sim[-1]),
                xy=(0.15, z_show[2]),
                xycoords="axes fraction",
            )
        ...
        if uncertainty:
            ...
            rot_max = (
                torch.sum(self.rbpf.rot.view(self.rbpf.n_particles, -1), dim=0)
                / self.rbpf.n_particles
            )
            rot_max_c = rot_max.clone()
            rot_max = rot_max.view(1, 37, 72, 72)
            rot_max, _ = torch.max(rot_max, dim=3)
            np.savez(
                self.log_dir + "/rot_{:06}.npz".format(step),
    ...
    def display_result(self, step, steps, refine=False):
        ...
        print(
            "     step {}/{}: translation error (filter)   = {:.4f} cm".format(
                step + 1, int(steps), filter_trans_error_bar * 100
            )
        )
        print(
            "     step {}/{}: xyz error (filter)           = ({:.4f}, {:.4f}, {:.4f})".format(
                step + 1,
                int(steps),
                self.log_err_tx[-1] * 100,
                self.log_err_ty[-1] * 100,
                self.log_err_tz[-1] * 100,
            )
        )
        print(
            "     step {}/{}: xyz rotation err (filter)    = ({:.4f}, {:.4f}, {:.4f})".format(
                step + 1,
                int(steps),
                self.log_err_rx[-1] * 57.3,
                self.log_err_ry[-1] * 57.3,
                self.log_err_rz[-1] * 57.3,
            )
        )
        print(
            "     step {}/{}: rotation error (filter)      = {:.4f} deg".format(
                step + 1, int(steps), filter_rot_error_bar * 57.3
            )
        )
        if not refine:
            return
        ...
        filter_rot_error_refine = abs(
            single_orientation_error(qco, mat2quat(rot_refine_f))
        )
        ...
        print(
            "     step {}/{}: translation error (refine)     = {:.4f} cm".format(
                step + 1, int(steps), filter_trans_error_refine * 100
            )
        )

        print(
            "     step {}/{}: xyz error (refine)           = ({:.4f}, {:.4f}, {:.4f})".format(
                step + 1,
                int(steps),
                log_err_tx_refine[-1] * 100,
                log_err_ty_refine[-1] * 100,
                log_err_tz_refine[-1] * 100,
            )
        )
        print(
            "     step {}/{}: xyz rotation err (refine)    = ({:.4f}, {:.4f}, {:.4f})".format(
                step + 1,
                int(steps),
                log_err_rx_refine[-1] * 57.3,
                log_err_ry_refine[-1] * 57.3,
                log_err_rz_refine[-1] * 57.3,
            )
        )
        print(
            "     step {}/{}: rotation error (refine)        = {:.4f} deg".format(
                step + 1, int(steps), filter_rot_error_refine * 57.3
            )
        )
    ...
    def save_log(self, sequence, filename, with_gt=True):
        if not self.log_created:
            self.log_pose = open(
                self.log_dir
                + "/Pose_{}_seq{}.txt".format(
                    self.target_obj_cfg.PF.TRACK_OBJ, sequence
                ),
                "w+",
            )
            if with_gt:
                self.log_pose_gt = open(
                    self.log_dir
                    + "/Pose_GT_{}_seq{}.txt".format(
                        self.target_obj_cfg.PF.TRACK_OBJ, sequence
                    ),
                    "w+",
                )
        ...
        self.log_pose.write(
            "{} {} {:.5f} {:.5f} {:.5f} {:.5f} {:.5f} {:.5f} {:.5f} {:.5f} {:.5f}\n".format(
                self.target_obj_cfg.PF.TRACK_OBJ,
                filename[0],
    ...
    def display_overall_result(self):
        ...
        print(
            "filter trans RMSE (x) = ",
            np.sqrt(np.mean(np.asarray(self.log_err_tx) ** 2)) * 1000,
        )
        print(
            "filter trans RMSE (y) = ",
            np.sqrt(np.mean(np.asarray(self.log_err_ty) ** 2)) * 1000,
        )
        print(
            "filter trans RMSE (z) = ",
            np.sqrt(np.mean(np.asarray(self.log_err_tz) ** 2)) * 1000,
        )
        print(
            "filter rot RMSE (x) = ",
            np.sqrt(np.mean(np.asarray(self.log_err_rx) ** 2)) * 57.3,
        )
        print(
            "filter rot RMSE (y) = ",
            np.sqrt(np.mean(np.asarray(self.log_err_ry) ** 2)) * 57.3,
        )
        print(
            "filter rot RMSE (z) = ",
            np.sqrt(np.mean(np.asarray(self.log_err_rz) ** 2)) * 57.3,
        )
    ...
    def initialize_poserbpf(
        ...
        self.rbpf.update_rot_star_R(
            quat2mat(self.rbpf_codepose[self.index_star[1]][3:])
        )
    ...
    def evaluate_particles(
        ...
        images_roi_cuda = trans_zoom_uvz_cuda(
            depth.detach(),
            uv,
            z,
            scale,
    ...
    def debug_gt_particles(self, n_gt_particles):
        ...
        self.rbpf.z[-n_gt_particles:] = (
            np.ones_like(self.rbpf.z[-n_gt_particles:]) * self.gt_z
        )
        self.rbpf.scale[-n_gt_particles:] = (
            np.ones_like(self.rbpf.z[-n_gt_particles:]) * 0.2
        )
    ...
    def refine_pose_and_shape(self, depth, intrinsics, refine_steps=50):
        ...
        if len(points_np) > NP_THRESHOLD:
            ...
            print(
                "*** NOT ENOUGH POINTS ON THE DEPTH IMAGE! ... SKIP REFINEMENT FOR THIS FRAME ***"
            )
            return
        # debug information
    ...
    def run_nocs_dataset(self, val_dataset, sequence):
        ...
        for inputs in val_generator:
            ...
            if len(inputs) == 8:
                ...
                self.mask = ndimage.binary_erosion(self.mask_raw, iterations=2).astype(
                    self.mask_raw.dtype
                )
            ...
            if step == 0:
                while not self.rbpf_ok:
                    ...
                    if self.prior_uv[0] > 0 and self.prior_uv[1] > 0:
                        print(
                            "[Initialization] Initialize PoseRBPF with detection center ... "
                        )
                        self.initialize_poserbpf(
                            images[0].detach(),
            ...
            if self.rbpf_ok:
                ...
                if self.target_obj_cfg.PF.USE_DEPTH:
                    ...
                    depth_data = None
                refine_start = 5  # 1000
                self.process_poserbpf(
                    images[0], intrinsics, depth=depth_data, run_deep_sdf=False
                ...
                #         plt.close()
                        ...
                        #         self.points_c,
                        #         self.label_gt,
                        #         self.latent_vec_pointnet,
                        #         self.latent_vec_refine,
                        #         self.evaluator,
                        #         self.decoder,
                        #         self.size_est / self.ratio,
                        #     )
                        #     self.save_shape_log(sequence, file_names)

                        #     shape_output_name = (
                        #         self.log_dir
                        #         + "/shape_pcd_est_{}_{}.ply".format(
                        #             self.target_obj_cfg.PF.TRACK_OBJ, step
                        #         )
                        #     )
                        ...
                        #     shape_output_name = (
                        #         self.log_dir
                        #         + "/shape_pcd_partial_{}_{}.ply".format(
                        #             self.target_obj_cfg.PF.TRACK_OBJ, step
                        #         )
                        #     )
                        #     pcd_ob = o3d.geometry.PointCloud()
                        #     pcd_ob.points = o3d.utility.Vector3dVector(
                        #         self.points_o_partial
                        #     )
