...
from ..deep_sdf import deep_sdf_decoder as arch
...
def load_decoder(experiment_directory, checkpoint_num=None, color_size=None, experiment_directory_color=None, parallel=True):
    ...
    if checkpoint_num != None:
        if color_size is not None:
            ...
            os.path.join(experiment_directory_color, "ModelParameters", str(checkpoint_num) + ".pth")
        )
            ...
            saved_model_state = torch.load(
            os.path.join(experiment_directory, "ModelParameters", str(checkpoint_num) + ".pth")
        )
