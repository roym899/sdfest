...
def vis_single(points):
    ...
    display_interval = 20
    ax.scatter(points[::display_interval, 0],
...
def visualize_pose_comparison(points_c_meas, points_all_np, T_co_init, T_co_opt):
    ...
    points_init = np.matmul(np.linalg.inv(T_co_init), points_c_meas.cpu().numpy().transpose()).transpose()
    points_opt = np.matmul(np.linalg.inv(T_co_opt), points_c_meas.cpu().numpy().transpose()).transpose()

    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    display_interval = 20
    ax.scatter(points_all_np[::display_interval, 0], points_all_np[::display_interval, 1], points_all_np[::display_interval, 2], color='green')
    ax.scatter(points_init[::display_interval, 0], points_init[::display_interval, 1], points_init[::display_interval, 2], color='red')
    ax.scatter(points_opt[::display_interval, 0], points_opt[::display_interval, 1], points_opt[::display_interval, 2], color='blue')
    ax.set_xlabel('X Label')
    ax.set_ylabel('Y Label')
    ax.set_zlabel('Z Label')
    min_coor = -0.2
...
def visualize_shape_comparison(points_gt, latent_gt, latent_tensor, latent_tensor_opt, evaluator, decoder, size_est):
    ...
    dist_opt = 10000
    try:
        points_init = evaluator.latent_vec_to_points(latent_tensor, num_points=10000, silent=True)
        points_opt = evaluator.latent_vec_to_points(latent_tensor_opt, num_points=10000, silent=True)
    ...
    return dist_init, dist_opt, points_init_metrics, points_opt_metrics   #dist_init, dist_opt, sdf_dist_init, sdf_dist_opt, sdf_dist_gt, points_init, points_opt, points_gt_sdf

def visualize_depth_pc(points, points_target):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    display_interval = 10
    ax.scatter(points[::display_interval, 0], points[::display_interval, 1], points[::display_interval, 2], color='green')
    ax.scatter(points_target[::display_interval, 0], points_target[::display_interval, 1], points_target[::display_interval, 2], color='red')
...
def visualize_pn_training_data(points, points_downsample, label_gt, evaluator):
    ...
    ax = fig.add_subplot(111, projection='3d')
    display_interval = 20
    ax.scatter(points_downsample[::display_interval, 0], points_downsample[::display_interval, 1], points_downsample[::display_interval, 2], color='green')
    ax.scatter(points[::display_interval, 0], points[::display_interval, 1], points[::display_interval, 2], color='red')
    ax.scatter(points_from_label_gt[::display_interval, 0], points_from_label_gt[::display_interval, 1], points_from_label_gt[::display_interval, 2], color='blue')
    ax.set_xlabel('X Label')
    ax.set_ylabel('Y Label')
    ax.set_zlabel('Z Label')
    min_coor = -1
...
def compute_chamfer_dist(points1_np, points2_np):
    ...
    dist_bidirectional = chamferDist(points1_torch, points2_torch, bidirectional=True).detach().cpu().item()
...
def visualize_shape_pc(points_gt_sdf, points_init, points_opt):
    ...
    ax = fig.add_subplot(111, projection='3d')
    display_interval = 20
    ax.scatter(points_gt_sdf[::display_interval, 0], points_gt_sdf[::display_interval, 1], points_gt_sdf[::display_interval, 2], color='green')
    ax.scatter(points_init[::display_interval, 0], points_init[::display_interval, 1], points_init[::display_interval, 2], color='red')
    ax.scatter(points_opt[::display_interval, 0], points_opt[::display_interval, 1], points_opt[::display_interval, 2], color='blue')
    ax.set_xlabel('X Label')
    ax.set_ylabel('Y Label')
