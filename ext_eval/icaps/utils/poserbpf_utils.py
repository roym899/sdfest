...
def trans_zoom_uvz_cuda(image, uvs, zs, scales, pf_fu, pf_fv, target_distance=3.5, out_size=128):
    image = image.permute(2, 0, 1).float().unsqueeze(0).cuda()

    bbox_u = target_distance * (1 / zs) / cfg.TRAIN.FU * pf_fu * out_size / image.size(3) * scales
    bbox_u = torch.from_numpy(bbox_u).cuda().float().squeeze(1)
    bbox_v = target_distance * (1 / zs) / cfg.TRAIN.FV * pf_fv * out_size / image.size(2) * scales
